...
### Server#origins(v:Function):Server
  ...
  * If `socket.io` is used together with `learnExpress`, the CORS headers will be affected only for `socket.io` requests. For Express can use [cors](https://github.com/expressjs/cors)
